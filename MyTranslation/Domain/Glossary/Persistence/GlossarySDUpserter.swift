...
extension Glossary.SDModel {
    ...
    final class GlossaryUpserter {
        ...
        private func key(of c: SDComponent) -> String { "\(c.pattern)|\(c.roles?.joined(separator: ",") ?? "-")" }
        private func key(of c: JSComponent) -> String { "\(c.pattern)|\(c.roles?.joined(separator: ",") ?? "-")" }
