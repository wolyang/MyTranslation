...
- [ ] (P1) 오버레이 패널에서 용어집 추가 기능 구현(`History/SPEC_OVERLAY_GLOSSARY_ADD.md` 참조):
  - [ ] Phase 1: 기본 플로우 (번역문 선택 → variants 추가) — 컨텍스트 메뉴/시트/TermEditor 진입점 추가 완료, 후보/컨텍스트 분석 미구현
  - [ ] Phase 2: 원문 선택 지원 (하이라이트 매칭된 기존 용어 편집/새 용어 추가 기본 흐름 구현, 번역문 범위 선택/추천 미구현)
  - [ ] Phase 3: 고급 기능 (번역문 선택 시 매칭되지 않은 용어 후보 추출·우선순위 표시, 후보 기반 변형 추가 UI; 원문 범위 입력/추천은 미구현)
  - [ ] Phase 4: 최적화 및 테스트
