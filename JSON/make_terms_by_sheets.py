...
def load_char_details(path):
    people = defaultdict(lambda: {
        "person_id": None,
        "name": {
            "family": {"source": [], "target": None},
            "given":  {"source": [], "target": None},
        },
        "aliases": []  # 임시로 list에 쌓고, 마지막에 target별 묶음
    })

    # alias 임시 버킷: pid -> target(str or None) -> set(sources)
    alias_bucket = defaultdict(lambda: defaultdict(set))
    ...
    for pid, pdata in people.items():
        ...
        aliases = []
        for tgt, src_set in alias_bucket[pid].items():
            aliases.append({
                "source": uniq_sorted(list(src_set)),
                "target": tgt  # None 허용
            })
