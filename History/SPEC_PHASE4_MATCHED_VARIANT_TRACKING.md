...
```
원문: "凯文说话" (凯文 × 1회)
번역: "케빈이 말할 때, Kevin도" (케빈 × 1회, Kevin × 1회)
→ Phase 1: "케빈" 정규화
→ Phase 4: "Kevin" → "케빈" 추가 정규화 필요
```
...
2. **1글자 변형이 무관한 텍스트를 오염시킴** (핵심 문제)
   - 예: 용어집 `奥` → target: "오", variants: ["울트라맨 오브"]
   - Phase 1에서 "울트라맨 오브" 매칭 성공
   - Phase 4에서 variants 전체 재검색 → **"오"도 검색됨**
   - "오늘은 날씨가" → "울트라맨 오브늘은 날씨가" ❌
...
**1차 패스 (Phase 1):**
```swift
// candidates = ["오", "오브", "울트라맨 오브"] (길이순 정렬)
// findNextCandidate() → "울트라맨 오브" 매칭 (가장 긴 것 우선)

out = "오의 힘, 오늘은"  // 정규화 완료
matchedVariantsByTarget["오"] = ["울트라맨 오브"]  // ✅ 실제 사용된 변형만 기록
protectedRanges = [NSRange(0, 1)]  // "오" 보호
```

**2차 패스 (Phase 4):**
```swift
// processedTargets = ["오"]
// matchedVariants = ["울트라맨 오브"]  // ← "오"는 포함되지 않음!

for variant in matchedVariants:  // ["울트라맨 오브"]
    // variant = "울트라맨 오브"
    // ✅ variant.count > 1 (7글자)
    // 검색 수행 → "오늘은"에서 "오" 매칭 안 됨 (matchedVariants에 없음)
