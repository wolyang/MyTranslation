...
- 코드를 제외한 **사용자 답변·문서·코드 주석은 모두 한국어로 작성**합니다.
...
- 로컬 에이전트는 직접 PR을 작성하지 않지만, 요청을 받으면 사람이 PR을 작성하기 위한 PR 제목과 본문을 생성해야 합니다. 아래의 원칙을 지켜 한국어로 작성하세요.

[PR 제목]
- 커밋 메시지와 동일하게, 변경의 요지가 드러나도록 작성합니다.

[PR 본문 작성 형식]
### 변경 이유
- 수정이 필요했던 배경과 문제 상황
- 기존 코드의 한계나 오류, 개선하려는 의도

### 변경 내용
- 파일별 주요 수정 사항을 bullet로 기술
  - 변경된 함수·클래스·프로퍼티 이름과 역할
  - "이전에는 ~했지만, 이제는 ~하게 됨" 형태로 기존 흐름 대비 변화 설명
  - 불필요한 중복 제거나 구조 단순화 등 개선 포인트 포함

### 변경의 의미
- 코드의 동작, 제어 흐름, 데이터 플로우가 어떻게 개선되었는지
- 유지보수성·안정성·성능 측면의 효과

### 영향 범위
- 영향받는 모듈·기능·UI
- 잠재적 리스크나 추가 검증 필요 영역

[추가 원칙]
- 함수나 프로퍼티 추가·삭제 시 그 변화가 전체 로직에 미치는 영향 설명
- 테스트 환경이 없어도 "테스트 미실행" 같은 문장은 쓰지 않습니다.
- 변경 성격에 따라 #로직, #UI, #빌드 등의 태그를 붙이면 좋습니다.

이 프롬프트의 목적은 변경 이유와 의도를 명확히 전달해, 다른 개발자가 PR만 읽어도 수정의 본질과 플로우 변화를 바로 이해할 수 있도록 하는 것입니다.

## 6. 규칙 변경 로그 (요약)

- 2025-11-20: Multi-Agent 규칙 문서 초기 버전 작성
- 2025-11-22: 코드 외 모든 커뮤니케이션(답변·문서·주석) 한국어 작성 규칙 추가 (언어 일관성)
