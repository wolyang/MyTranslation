...
## 검토 메모
- Phase별로 `MyTranslation.xcodeproj` 그룹/소스 경로, 스킴, `MyTranslation.xctestplan`을 함께 갱신하고 빌드 확인을 필수 체크로 추가합니다(대규모 `git mv` 후 깨진 참조 방지).
- 실제로 단일 타겟인지, SPM/멀티 타겟으로 나눌지 결정한 뒤 import 변경 범위를 최소화합니다. 단일 타겟이라면 폴더 이동만으로 충분하므로 대량 import 수정은 지양합니다.
- 파일 이동/이름 변경과 로직 분할을 같은 단계에 섞지 않고, “순수 이동(또는 이름 변경) → 분할” 순으로 별도 커밋을 만들어 빌드 브레이크 원인 추적성을 높입니다.
- 500줄 이상 파일 분할 시 `private`/`fileprivate` 의존이 깨질 수 있으므로, 분할 커밋에서 접근 제어 상향(필요 최소 범위)을 명시적으로 처리하고 로직 변경 없음도 함께 기록합니다.
- `Shared/Models`와 `Shared/Services`로 이동하는 `BrowsingHistory`/`HistoryStore`가 사용하는 UserDefaults 키/파일 경로 네임스페이스가 이동에 따라 바뀌지 않도록 점검 항목을 추가합니다.
- Phase 11에서 남는 빈 그룹/죽은 경로를 pbxproj에서 제거하는 정리 스텝을 포함해 최종 빌드 스크립트·리소스 경로를 재검증합니다.
- 문서 업데이트는 구조 반영 전용 커밋으로 묶고, Phase 10 이후 최종 스냅샷을 한 번 더 갱신해 충돌 가능성을 줄입니다.
